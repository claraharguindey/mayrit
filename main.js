...
function buildGrid() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    cells.push({ el: cell, img, index: i });
  }
  layout();
}
...
function layout() {
  ...
  cells.forEach((c, i) => {
    ...
    const virtualRow = row + Math.floor(-offsetY / rowH);
    const itemIndex = (virtualRow * cols) + col;
    const item = pool[((itemIndex % pool.length) + pool.length) % pool.length];
    const num = String(item.n).padStart(2, '0');
    const id = `${item.cat}-${num}`;
    const src = `./${item.cat}/${id}.webp`;
    if (c.img.dataset.src !== src) {
      c.img.dataset.src = src;
      c.img.src = src;
    }
    c.el.dataset.label = `${item.cat.toUpperCase()} — ${id}`;
  });
...
function buildPanelHTML(num, title, subtitle, body, meta) {
  return `
    <div class="panel-inner">
      ...
      <div class="panel-line" style="margin-bottom:1.4rem"><p class="panel-subtitle">${subtitle}</p></div>
